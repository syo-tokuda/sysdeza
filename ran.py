int moisture_data  土壌水分センサの値を保存する変数。初期値は’0’。
int temperature_data  土壌温度センサの値を保存する変数。初期値は’0’。
int salt_content　コンポスト内の塩分量を保存する変数。初期値は’0’。
int moisture_judge　土壌水分センサの値から判断した，土の状態を保存する変数。初期値は’0’。
int temperature_judge_1　土壌温度センサの値から判断した，土の状態を保存する変数。土の温度が60度以上となっている状態が7日間続いたかを確認する。初期値は’0’。
int temperature_judge_2　土壌温度センサの値から判断した，土の状態を保存する変数。土の温度が65度に達した後外気温付近まで低下したかを確認する。初期値は’0’。
bool automatic　自動攪拌機能がONかOFFかを保存する変数。初期値は’偽’。
bool Arduino_connect　Arduinoと通信できたかを保存する変数。初期値は’偽’。
bool moisture_connect　 土壌水分センサの値が読めたかを保存する変数。初期値は’偽’。
bool temperature_connect　土壌温度センサの値が読めたかを保存する変数。初期値は’偽’。
bool barcode_connect　 バーコードリーダーと通信できたかを保存する変数。初期値は’偽’。
bool barcode_collation　バーコードリーダーで読み取ったコードがデータベースにあるかを保存する変数。初期値は’真’。
